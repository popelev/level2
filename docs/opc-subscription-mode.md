...
3. On each tick `pollOnce` → `pollBatch`: OPC UA **Read** in batches of **`maxNodesPerRead = 100`** (guard against `BadTooManyOperations` on Siemens and others).
...
sequenceDiagram
  ...
  loop every min(interval_ms)
    Drv->>PLC: Read (batches ≤100 NodeId)
    PLC-->>Drv: DataValues
    loop each tag
      Drv->>Fan: Sample
...
| Component | Path |
|-----------|------|
| Poll-as-Subscribe | `internal/driver/opcua/driver.go` (`Subscribe`, `pollOnce`, `maxNodesPerRead`) |
